...
export default function GlobalError({ error, reset }: { error: Error; reset: () => void }) {
  return (
    <html lang="ja">
      <body className="bg-black text-white min-h-screen antialiased flex flex-col items-center justify-center px-6 text-center">
        <p className="text-[120px] font-bold text-white/10 leading-none">500</p>
        <h1 className="text-xl font-bold text-white mt-4 mb-2">重大なエラーが発生しました</h1>
        <p className="text-sm text-gray-500 mb-8">{error.message || 'アプリケーションの読み込みに失敗しました。'}</p>
